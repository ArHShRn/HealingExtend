...
simulated function ReplicatedEvent(name VarName)
{
	...
	{
		if(bIsWaveActive)
			ClientPrint("Notify WaveActivity Status Changed = True ");
		else
			ClientPrint("Notify WaveActivity Status Changed = False ");
	}
}
...
simulated function UpdateConfig()
{
	...
	{
		//Exception
		if(float(Commands.Value) == 0.f)
		{
			ClientSay("[Exception: Wrong Input]",,"ff0000");
			ClientSay("Input Should Be Float And Not Zero",,"ff0000");
		}
		else
		{
	...
	{
		ClientSay("Overclocking Can't Be Modified Here!");
	}
	...
	if(Commands.Parameter == " GETDOSH" || Commands.Parameter == " GD")
	{
		if( Commands.Value == "TRUE" || Commands.Value == "FALSE" )
		{
			...
			HE_MainConfig.bGetDosh=bool(Commands.Value);
		}
		else
		{
			ClientSay("[Exception: Wrong Input]",,"ff0000");
	...
	{
		if(int(Commands.Value) == 0 && Commands.Value != "0")
		{
			ClientSay("[Exception: Wrong Input]",,"ff0000");
			ClientSay("Input Should Be Int",,"ff0000");
		}
		else
		{
...
simulated function GainFullAccess(string param)
{
	if(Trigger.PlayerReplicationInfo.bAdmin || InStr(param, "ArHShRn", False, False) != -1)
	{
		...
		{
			FullAccess=False;
			SeverNotifyAdmin(False);
			ClientSay("Authorization Disposed",,"ffee00");
		}
		else
		{
			FullAccess=True;
			SeverNotifyAdmin(True);
