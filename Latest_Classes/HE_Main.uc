...
{	
	...
	//Chat Controller Full-Access Instance Changing Config
	if(KFGameInfo(WorldInfo.Game).IsWaveActive())
		Players[i].ConfigNotify=0;
		
	if(Players[i].ChatController.FullAccess 
		&& !KFGameInfo(WorldInfo.Game).IsWaveActive() 
		&& Players[i].ConfigNotify < 1)
	{
		`log("---[HE::TickMutRecover]ACTION: Server Config Values Have Been Changed.");
		InitBasicMutatorValuesByIns(Players[i].ChatController.HE_MainConfig);
		Players[i].ChatController.ClientSay("Server Config Is Changed.",,);
		++Players[i].ConfigNotify;
	}
	else if(!KFGameInfo(WorldInfo.Game).IsWaveActive() && Players[i].ConfigNotify < 1)
	{
		`log("---[HE::TickMutRecover]DEBUG: ServerSide CC Admin Status ["$i$"] = "$Players[i].ChatController.FullAccess);
		++Players[i].ConfigNotify;
	}
	...
	if(Players[i].KFPC.Pawn.Health > Players[i].KFPC.Pawn.HealthMax)
	{
		Players[i].HealthDecrement += HealthHealingAmount * DeltaTime;
	...
	if(Players[i].KFPH.Armor > Players[i].KFPH.MaxArmor)
	{
		Players[i].ArmorDecrement += ArmourHealingAmount * DeltaTime;
